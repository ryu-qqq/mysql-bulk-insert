...
## 테스트 조건:
- **상품 그룹**: 1,000개
- **상품**: 각 상품 그룹마다 **10개**의 상품을 고정으로 할당하여 총 **10,000개**의 상품 데이터를 생성
- JPA와 JDBC 각각 성능을 측정하여 비교

#### 결과 요약

## 실험 결과:
| **방식** | **100개 삽입 시 (ms)** | **1,000개 삽입 시 (ms)** | **10,000개 삽입 시 (ms)** | **총 실행 시간 (ms)** | **평균 삽입 시간 (ms)** |
|----------|----------------------|------------------------|-------------------------|----------------------|------------------------|
| **JPA**  | 500ms                | 1470ms                 | 8545ms                  | 1240ms               | 1.24ms                 |
| **JDBC** | 74ms                 | 182ms                  | 1419ms                  | 958ms                | 0.96ms                 |
...
### 결론 및 발견 사항
- **JDBC 방식의 우수성**: JPA 대비 **약 4배**의 성능 개선을 확인했습니다.
